...
def drive_sim(
    ...
    if use_vehicle_adaptor:
        ...
        vehicle_adaptor = utils.VehicleAdaptor()
        vehicle_adaptor.set_NN_params(
            ...
            vehicle_model.lstm_1.weight_ih_l0.detach().numpy().astype(np.float64),
            vehicle_model.lstm_1.weight_hh_l0.detach().numpy().astype(np.float64),
            vehicle_model.lstm_2.weight_ih_l0.detach().numpy().astype(np.float64),
            vehicle_model.lstm_2.weight_hh_l0.detach().numpy().astype(np.float64),
            ...
            vehicle_model.lstm_1.bias_hh_l0.detach().numpy().astype(np.float64),
            vehicle_model.lstm_1.bias_ih_l0.detach().numpy().astype(np.float64),
            vehicle_model.lstm_2.bias_hh_l0.detach().numpy().astype(np.float64),
            vehicle_model.lstm_2.bias_ih_l0.detach().numpy().astype(np.float64),
    ...
    for i in range(t_eval.size):
        if i % ctrl_freq == 0:  # update u_opt
            ...
            if use_vehicle_adaptor:
                if states_ref_mode == "controller_d_input_schedule" and control_type == ControlType.mpc:
                    vehicle_adaptor.set_controller_d_input_schedule(controller.nominal_inputs[:,0], controller.nominal_inputs[:,1])
                if states_ref_mode == "controller_prediction" and control_type == ControlType.mpc:
                    vehicle_adaptor.set_controller_prediction(controller.nominal_traj[:,2],controller.nominal_traj[:,4], controller.nominal_traj[:,5])
                u_opt_adjusted = vehicle_adaptor.get_adjusted_inputs(x_current, u_opt[0], u_opt[1])
            else:
        ...
        acc_des_queue.append(u_opt_adjusted[0])
        steer_des_queue.append(u_opt_adjusted[1])
