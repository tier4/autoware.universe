...
#ifndef EUCLIDEAN_CLUSTER__FEC_H_
#define EUCLIDEAN_CLUSTER__FEC_H_

#endif  // EUCLIDEAN_CLUSTER__FEC_H_
...
std::vector<pcl::PointIndices> FEC(
  pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, int min_component_size, double tolorance, int max_n)
{
  unsigned long i, j;
  ...
  for (i = 0; i < cloud_size; i++) {
    ...
    if (marked_indices[i] == 0)  // reset to initial value if this point has not been manipulated
    {
      ...
      for (j = 0; j < pointIdx.size(); j++) {
        ...
        /*
         * Each domain point, as well as all points in the same cluster, is uniformly assigned this
         * label
         */
