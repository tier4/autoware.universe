...
class PythonSimulator:
    ...
    def common_initialization(self):
        ...
        if self.accel_brake_map_sim_path is not None:
            ...
            self.use_accel_brake_map = True
    def initialize_slalom_drive(self,initial_error):
    ...
    def F_sim(self, states, inputs):
        ...
        return states + states_dot * self.sim_dt
    ...
    def drive_sim(
        ...
        if control_type == ControlType.mpc:
            self.initialize_slalom_drive(initial_error)
        ...
        while True:
            if simulation_num % self.control_step == 0:
                if control_type == ControlType.mpc:
                    ...
                    if USE_SMART_MPC_PACKAGE:
                        ...
                        u_opt = self.controller.compute_optimal_control(t_current, self.states_current, X_des[:, :6].T)

                    #print("ilqr time: ", time.time() - start_ilqr_time)

                elif control_type == ControlType.pp_eight:
                    self.pp_gain_updater.state_queue_updater(
                        self.states_current[vel_index], self.states_current[yaw_index], self.states_current[acc_index], self.states_current[steer_index]
                    )
                    (
                        target_position,
                        target_yaw,
                        ...
                        self.states_current, self.trajectory_position_data, self.trajectory_yaw_data, self.previous_pp_index
                    )
                    ...
                    if not self.data_collection:
                        ...
                        if part == "linear_positive" or part == "linear_negative":
                            current_vel = self.states_current[2]
                    ...
                    u_opt = self.pure_pursuit_controller.pure_pursuit_control(
                        pos_xy_obs=self.states_current[:2],
                        pos_yaw_obs=self.states_current[yaw_index],
                        longitudinal_vel_obs=self.states_current[vel_index],
                ...
                if use_vehicle_adaptor:
                    ...
                    u_opt_adjusted = self.vehicle_adaptor.get_adjusted_inputs(t_current,self.states_current, u_opt[0], u_opt[1])
                ...
                if (self.accel_map_control.is_map_used(self.states_current[vel_index],u_opt_adjusted[0])):
                    accel_input = self.accel_map_control.get_actuation_cmd(self.states_current[vel_index],u_opt_adjusted[0])
                    brake_input = - 1e-10
                else:
                    accel_input = - 1e-10
                    brake_input = self.brake_map_control.get_actuation_cmd(self.states_current[vel_index],u_opt_adjusted[0])
                if control_type == ControlType.mpc:
                    self.log_updater.update(t_current, self.states_current, u_opt,u_opt_adjusted, accel_input, brake_input)
                else:
                    self.log_updater.update(t_current, self.states_current, u_opt,u_opt_adjusted, accel_input, brake_input, part)
            ...
            u_actual_input[1] = self.steer_input_queue.pop(0)
            ...
            if control_type == ControlType.mpc and (simulation_num % 1000 == 999 or break_flag):
                self.save_slalom_drive_record(t_current, save_dir, use_vehicle_adaptor)
    ...
    def save_slalom_drive_record(self, t_current, save_dir, use_vehicle_adaptor):
            ...
            if use_vehicle_adaptor:
                vehicle_adaptor_inputs_hist = np.array(self.log_updater.vehicle_adaptor_cmd_history)[:,2:]
    ...
    def get_mpc_trajectory(self, t_current):
        """Calculate the target trajectory to be used in MPC from the given data."""
        nearest_index = np.argmin(
            ((self.trajectory_data[:, 1:3] - self.states_current[:2].reshape(1, 2)) ** 2).sum(axis=1)
            + 1e-5 * (self.trajectory_data[:,0]-t_current)**2
        )
