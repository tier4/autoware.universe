  ...
  const geometry_msgs::msg::Pose & ego_pose) const
{
  ...
  if (zero_vel_idx) {
    ...
    const double ratio =
      calcRatio(output_traj_points, nearest_seg_idx, zero_vel_traj_pose.position);
    ...
    if (0.0 <= ratio && ratio <= 1.0) {
      ...
      geometry_msgs::msg::Point stop_pos;
      stop_pos.x = interpolation::lerp(prev_pose.position.x, next_pose.position.x, ratio);
      stop_pos.y = interpolation::lerp(prev_pose.position.y, next_pose.position.y, ratio);
      stop_pos.z = interpolation::lerp(prev_pose.position.z, next_pose.position.z, ratio);

      if (geometry_utils::isSamePoint(prev_pose, stop_pos)) {
        output_traj_points.at(nearest_seg_idx).longitudinal_velocity_mps = 0.0;
      } else if (geometry_utils::isSamePoint(next_pose, stop_pos)) {
        output_traj_points.at(nearest_seg_idx + 1).longitudinal_velocity_mps = 0.0;
      } else {
        TrajectoryPoint additional_traj_point;
        additional_traj_point.pose.position = stop_pos;
