  ...
  Eigen::VectorXd * Uex, const MPCTrajectory & traj)
{
  ...
  const auto get_adaptive_steer_rate_lim = [&] (const double curvature) {
    for (size_t j = 0; j < m_curvature_list_for_steer_rate_lim.size(); ++j) {
      if (curvature < m_curvature_list_for_steer_rate_lim.at(j)) {
	return m_steer_rate_lim_list.at(j);
      }
    }
  ...
  */
  ...
  VectorXd lbA = VectorXd::Constant(DIM_U_N, -m_steer_lim);  // min steering angle
  VectorXd ubA = VectorXd::Constant(DIM_U_N, m_steer_lim);  // min steering angle
  /*
  std::cerr << "======================== 0 ==========================" << std::endl;
  VectorXd lb = VectorXd::Constant(DIM_U_N, -m_steer_lim);  // min steering angle
  VectorXd ub = VectorXd::Constant(DIM_U_N, m_steer_lim);   // max steering angle
  ...
  for (int i = 0; i < DIM_U_N; ++i) {
    ...
    const double adaptive_delta_steer_rate_lim = adaptive_steer_rate_lim * prediction_dt;
    lbA(i) = -adaptive_delta_steer_rate_lim;
    ubA(i) = adaptive_delta_steer_rate_lim;
  }
  std::cerr << "======================== 2 ==========================" << std::endl;  
