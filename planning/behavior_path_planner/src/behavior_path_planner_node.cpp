...
PullOverParameters BehaviorPathPlannerNode::getPullOverParam()
{
  ...
  //   p.backward_goal_search_length = declare_parameter<double>(ns +
  //   "backward_goal_search_length"); p.goal_search_interval = declare_parameter<double>(ns +
  //   "goal_search_interval"); p.longitudinal_margin = declare_parameter<double>(ns +
  //   "longitudinal_margin"); p.max_lateral_offset = declare_parameter<double>(ns +
  //   "max_lateral_offset"); p.lateral_offset_interval = declare_parameter<double>(ns +
  //   "lateral_offset_interval"); p.ignore_distance_from_lane_start =
  ...
  //   // hazard
  //   p.hazard_on_threshold_distance = declare_parameter<double>(ns +
  //   "hazard_on_threshold_distance"); p.hazard_on_threshold_velocity =
  //   declare_parameter<double>(ns + "hazard_on_threshold_velocity");
  ...
  //   const std::string parking_policy_name = declare_parameter<std::string>(ns +
  //   "parking_policy"); if (parking_policy_name == "left_side") {
  //     p.parking_policy = ParkingPolicy::LEFT_SIDE;
  //   } else if (parking_policy_name == "right_side") {
  //     p.parking_policy = ParkingPolicy::RIGHT_SIDE;
  //   } else {
  //     RCLCPP_ERROR_STREAM(
  //       get_logger(), "[pull_over] invalid parking_policy: " << parking_policy_name <<
  //       std::endl);
  //     exit(EXIT_FAILURE);
  //   }
  // }
  ...
  //   p.common_parameters.turning_radius_size = std::max(p.common_parameters.turning_radius_size,
  //   1);
  ...
  //   p.common_parameters.lateral_goal_range = declare_parameter<double>(ns +
  //   "lateral_goal_range"); p.common_parameters.longitudinal_goal_range =
  ...
  //   p.rrt_star_parameters.max_planning_time = declare_parameter<double>(ns +
  //   "max_planning_time"); p.rrt_star_parameters.neighbor_radius = declare_parameter<double>(ns +
  //   "neighbor_radius"); p.rrt_star_parameters.margin = declare_parameter<double>(ns + "margin");
  // }
...
PullOutParameters BehaviorPathPlannerNode::getPullOutParam()
{
  ...
  //   p.geometric_pull_out_velocity = declare_parameter<double>(ns +
  //   "geometric_pull_out_velocity"); p.arc_path_interval = declare_parameter<double>(ns +
  //   "arc_path_interval"); p.lane_departure_margin = declare_parameter<double>(ns +
  //   "lane_departure_margin"); p.backward_velocity = declare_parameter<double>(ns +
  //   "backward_velocity"); p.pull_out_max_steer_angle = declare_parameter<double>(ns +
  //   "pull_out_max_steer_angle");  // 15deg
  ...
  //   p.backward_path_update_duration = declare_parameter<double>(ns +
  //   "backward_path_update_duration"); p.ignore_distance_from_lane_end =
  //   declare_parameter<double>(ns + "ignore_distance_from_lane_end");
  //   // drivable area
  //   p.drivable_area_right_bound_offset =
  //     declare_parameter<double>(ns + "drivable_area_right_bound_offset");
  //   p.drivable_area_left_bound_offset =
