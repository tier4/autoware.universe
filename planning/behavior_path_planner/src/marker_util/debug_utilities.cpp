...
MarkerArray createShiftGradMarkerArray(
  const std::vector<double> & grad, const PathWithLaneId & reference, std::string && ns,
  const float & r, const float & g, const float & b)
{
  ...
  Marker marker = createDefaultMarker(
    ...
    createMarkerScale(0.3, 0.3, 0.3), createMarkerColor(r, g, b, 0.9));

  for (size_t i = 0; i < grad.size(); ++i) {
    std::ostringstream string_stream;
    string_stream << "g:" << grad.at(i);
    marker.text = string_stream.str();
    marker.pose = calcOffsetPose(reference.points.at(i).point.pose, 0.0, 10.0 * grad.at(i), 0.0);
    marker.id = i;
